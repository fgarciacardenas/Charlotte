...
class Joystick {
    ...
    Joystick(ros::NodeHandle &n) {
        sub = n.subscribe("joy", 10, &Joystick::callback, this);
        this->axes.assign(6,0.2);
        this->buttons.assign(6,0.2);
    }
    ...
    void get_values(std::vector<float> &axesin, std::vector<int32_t> &buttonsin) {
        axesin.resize(axes.size(), 0);
        buttonsin.resize(buttons.size(), 0);
        for (unsigned int j = 0; j < axes.size(); j++)
            axesin[j] = axes[j];
        for (unsigned int i = 0; i < buttons.size(); ++i)
            buttonsin[i] = buttons[i];
    }
};
...
void vectores(double& angle, double& dx, double& dy, double& distance, VectorXd& posd);
void select_dir(int& casos, Vector2d& direction);
...
int main(int argc, char **argv) {
    ...
    Joystick control(n);
    std::vector<float> axesin;
    std::vector<int32_t> buttonsin;
    ...
    while (ros::ok()) {  
        control.get_values(axesin, buttonsin);
        speed = 0.001 + (0.01 - 0.001) * ((control.axes[3] - (-1)) / (1 - (-1)));

        // Desired position[posb, leg1, leg2, leg3, leg4, rotb]
        if (control.buttons[0] == 1) {
            xd.segment(0, 3)  << pos.posb(0) + control.axes[0]*speed, pos.posb(1) + control.axes[1]*speed, pos.posb(2) + control.axes[5]*speed;
        }
        else if (control.buttons[1] == 1) {
            xd.segment(3, 3)  << pos.leg1(0) + control.axes[0]*speed, pos.leg1(1) + control.axes[1]*speed, pos.leg1(2) + control.axes[5]*speed;
        }
        else if (control.buttons[2] == 1) {
            xd.segment(6, 3)  << pos.leg2(0) + control.axes[0]*speed, pos.leg2(1) + control.axes[1]*speed, pos.leg2(2) + control.axes[5]*speed;
        }
        else if (control.buttons[3] == 1) {
            xd.segment(9, 3)  << pos.leg3(0) + control.axes[0]*speed, pos.leg3(1) + control.axes[1]*speed, pos.leg3(2) + control.axes[5]*speed;
        }
        else if (control.buttons[4] == 1) {
            xd.segment(12, 3)  << pos.leg4(0) + control.axes[0]*speed, pos.leg4(1) + control.axes[1]*speed, pos.leg4(2) + control.axes[5]*speed;
        }
        else if (control.buttons[5] == 1) {
            xd.segment(15, 3)  << pos.rotb(0) + control.axes[0]*speed*(pi/180), pos.rotb(1) + control.axes[1]*speed*(pi/180), pos.rotb(2) + control.axes[5]*speed*(pi/180);
        }
        ...
        //for (int i = 3; i < rvizq.size() - 3; i++)
        //{
        //    rviz_val.position.push_back(rvizq[i]);
        //}
...
void walking(VectorXd& posd, Legs& pos, MatrixXd& final, int& ind) {
    ...
    int casos;
    double angle, dx, dy, distance;
    double modulo, residuo, num, resix, resiy;
    Vector2d direction;
    VectorXd M(18);
    
    // Pasos previos
    // Se halla el vector direccion
    vectores(angle, dx, dy, distance, posd);
    direction << dx, dy;
    ...
    //double parax1 = modulo * cos(angle) * direction(0);
    //double paray1 = modulo * sin(angle) * direction(1);
    //double resta = sqrt(pow((parax1/4),2) + pow((paray1/4),2));

    // Inicio
    //distance = distance - resta;

    // Cantidad de pasos
    //gait_step(modulo, residuo, num, distance);
    
    // Desplazamiento en funcion del angulo hallado
    double parax = modulo * cos(angle) * direction(0);
    double paray = modulo * sin(angle) * direction(1);

    // Residuos en el desplazamiento
    //resix = residuo * cos(angle) * direction(0);
    //resiy = residuo * sin(angle) * direction(1);

    M.segment(0, 3)  << pos.posb(0), pos.posb(1), pos.posb(2);
    M.segment(3, 3)  << pos.leg1(0), pos.leg1(1), pos.leg1(2);
    M.segment(6, 3)  << pos.leg2(0), pos.leg2(1), pos.leg2(2);
    M.segment(9, 3)  << pos.leg3(0), pos.leg3(1), pos.leg3(2);
    M.segment(12, 3) << pos.leg4(0), pos.leg4(1), pos.leg4(2);
    M.segment(15, 3) << pos.rotb(0), pos.rotb(0), pos.rotb(0);

    select_dir(casos, direction);
    ...
    switch(casos) {
        case 1:
            ...
            ind = 6; // Indice de la matriz IMPORTANTE
